...
export default function FavoritesList() {
  ...
  const {
    ...
    genre: activeFilter && activeFilter !== 'Tous' ? activeFilter : undefined,
    sortBy,
    sortOrder,
  });

  // Hook pour charger tous les genres des favoris
  const { allGenres } = useFavoritesGenres();
  ...
  const safeFavoritesSongsData = favoritesSongsData || {
    songs: [],
    pagination: {
      currentPage: 1,
      totalPages: 0,
  ...
  const goToNextPage = () => {
    if (safeFavoritesSongsData.pagination.hasNextPage) {
      setCurrentPage(currentPage + 1);
    }
  };

  const goToPreviousPage = () => {
    if (safeFavoritesSongsData.pagination.hasPreviousPage) {
      setCurrentPage(currentPage - 1);
    }
  };

  // Création des filtres disponibles à partir de tous les genres des favoris
  ...
  return (
    <div className="w-full">
      <div className="space-y-4">
        ...
        >
          <div>
            {safeFavoritesSongsData.pagination.totalSongs} morceau
            {safeFavoritesSongsData.pagination.totalSongs !== 1 ? 'x' : ''}{' '}
            favori
            {safeFavoritesSongsData.pagination.totalSongs !== 1 ? 's' : ''}
          </div>
        ...
        {/* Pagination */}
        {(safeFavoritesSongsData.pagination.totalSongs > 0 || isLoading) && (
          <div className={styles.paginationContainer}>
            <div className={styles.paginationControls}>
              <button
                className={`${styles.paginationButton} ${
                  !safeFavoritesSongsData.pagination.hasPreviousPage ||
                  isLoading
              ...
              <div className={styles.paginationPageInfo}>
                {isLoading
                  ? 'Chargement...'
                  : `Page ${safeFavoritesSongsData.pagination.currentPage} sur ${safeFavoritesSongsData.pagination.totalPages}`}
              </div>
              <button
                className={`${styles.paginationButton} ${
                  !safeFavoritesSongsData.pagination.hasNextPage || isLoading
                    ? styles.paginationButtonDisabled
                    : ''
                }`}
        ...
        {/* État vide */}
        {!isLoading && safeFavoritesSongsData.pagination.totalSongs === 0 && (
          <div className={styles.emptyState}>
            ...
            <h3 className={styles.emptyTitle}>
              {debouncedSearchQuery || activeFilter
                ...
                : 'Aucune chanson favorite'}
            </h3>
            <p className={styles.emptyDescription}>
              {debouncedSearchQuery || activeFilter
                ? 'Aucune chanson ne correspond à vos critères de recherche. Essayez de modifier vos filtres ou votre recherche.'
                : "Vous n'avez encore aucune chanson favorite. Ajoutez des chansons à vos favoris pour les voir ici !"}
            </p>
            {debouncedSearchQuery || activeFilter ? (
              <button
